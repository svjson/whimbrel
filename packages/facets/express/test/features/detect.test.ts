...
describe('detect', () => {
  it('should not detect Express when no package.json is present', async () => {
    // Given
  ...
  it('should not detect Express when package.json does not list express as dependency', async () => {
    // Given
    const ctx = await memFsContext()
    const root = await createDirectory(['tsconfig.json'], ctx.disk)
  ...
  it('should detect Express when package.json list express as dependency', async () => {
    // Given
    const ctx = await memFsContext()
    const root = await createDirectory(['tsconfig.json'], ctx.disk)
    const pkgJson = new PackageJSON({
