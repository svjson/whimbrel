...
export class ConsoleAppender implements ApplicationLog {
  ...
  #indentAndFormat(args: any[]): any[] {
    if (typeof args[0] === 'string') {
      return [indent(this.indentation, args[0]), ...args.slice(1)]
    }
    return [indent(this.indentation, ''), ...args]
  }
  ...
  info(...args: any[]) {
    this.#deferSpinner(() => {
      console.log(...this.#indentAndFormat(args))
    })
  }

  warn(...args: any[]) {
    this.#deferSpinner(() => {
      console.warn(...this.#indentAndFormat(args))
    })
  }

  error(...args: any[]) {
    this.#deferSpinner(() => {
      console.error(...this.#indentAndFormat(args))
    })
  }

  debug(...args: any[]) {
    this.#deferSpinner(() => {
      console.debug(...this.#indentAndFormat(args))
    })
  }
